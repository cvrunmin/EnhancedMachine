...
public abstract class AbstractFurnaceScreenMixin extends ContainerScreen {
    ...
    @Shadow
    @Final
    private static ResourceLocation field_214089_l;
    @Shadow
    ...
    private Widget modifyRecipeButton(Widget old){
        ...
        if(count > 1){
            return new ImageButton(this.guiLeft + 78, this.guiTop + 16, 20, 18, 0, 0, 19, field_214089_l, (p_214087_1_) -> {
    ...
    @ModifyArg(method = "render", at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/recipebook/AbstractRecipeBookGui;renderGhostRecipe(IIZF)V"), index = 2)
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < count * 2 + 1; i++) {
            ...
            if (i == 0) { //Fuel slot
                this.blit(left + (slot != null ? slot.xPos - 1 : 55), top + (slot != null ? slot.yPos - 1 : 52), 194, 48, 18, 18);
            } else if (i % 2 == 1) { //Input
                if (slot == null) continue;
                this.blit(left + slot.xPos - 1, top + slot.yPos - 1, 176, 48, 18, 18);
            } else { //Output
                if (slot == null) continue;
                this.blit(left + slot.xPos - (count > 4 ? 1 : 5), top + slot.yPos - (count > 4 ? 1 : 5), count > 4 ? 202 : 176, 66, count > 4 ? 18 : 26, count > 4 ? 18 : 26);
            }
        }
        //Furnace fuel left
        this.blit(left + (count == 1 ? 56 : /*79*/ 89), top + (count == 1 ? 36 : /*46*/ 56), 190, 0, 14, 14);
        //Furnace progress
        this.blit(left + (count == 1 ? 79 : 76), top + (count == 1 ? 34 : 29 + 5), 176, 30, 24, 16);

        if (((AbstractFurnaceContainer) container).isBurning()) {
            int k = ((AbstractFurnaceContainer)this.container).getBurnLeftScaled();
            this.blit(left + (count == 1 ? 56 : 89), top + (count == 1 ? 36 : 56) + 12 - k, 176, 12 - k, 14, k + 1);
        }

        int l = ((AbstractFurnaceContainer)this.container).getCookProgressionScaled();
        this.blit(left + (count == 1 ? 79 : 76), top + (count == 1 ? 34 : 29 + 5), 176, 14, l + 1, 16);
    }
}
